...
# Criando o modelo base com a variável am (categórica)
?lm
...
#Usando somente uma var tem um risco grande de perda de informações. Não da p dizer que o carro é mais economico observando apenas se
# é manual ou automatico.


# Por isso usaremos a Regressão Linear Múltipla

#Assim podemos saber qual vai ser a combinação de var's que melhor irá prever a eficiência de gasto de combustivel.
# Com quais preditores aumentam nossa precisão, tendencias no grafico para determinar a formula ideal para previsão
...
# no modelo, cuja escolha é do cientista de dados.
...
# Isso mostra que, além da transmissão, o peso do veículo (wt) e a velocidade de aceleração (qsec) têm a maior relação com a explicação da variação em mpg. 
# O R^2 é de 85%, o que significa que o modelo explica 85% da variação em mpg, indicando que é um modelo robusto e altamente preditivo.
...
# Ao plotar resíduos versus os valores ajustados, procuramos qualquer tipo de padrão. O mesmo ocorre com os valores ajustados versus os padronizados, quando estamos plotando 
# uma função dos resíduos padronizados. O gráfico abaixo mostra que não existe um padrão específico nos resíduos.
